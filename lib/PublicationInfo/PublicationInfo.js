...
const getCol = ({ col, pubInfo }) => {
  return <Col key={col} xs={4}>
    <KeyValue
      data-testid={`iso18626-pubInfo-${col}`}
      label={<FormattedMessage id={`ui-ill-components.iso18626.pubInfo.${col}`} />}
      value={pubInfo[col]}
    />
  </Col>;
};
...
const PublicationInfo = ({
  ...
  return (
    <Layout className="margin-start-gutter">
      <Accordion
        id="pubInfoSection"
        label={<FormattedMessage id="ui-ill-components.iso18626.pubInfo.heading" />}
      >
        {getRows(pubInfo)}
      </Accordion>
    </Layout>
