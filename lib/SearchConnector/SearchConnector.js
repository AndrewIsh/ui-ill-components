...
const SearchConnector = ({
  ...
  const footer = (
    <ModalFooter>
      <Button buttonStyle="primary" marginBottom0>
        <FormattedMessage id="ui-ill-ra.button.placeRequest" />
      </Button>
      <Button onClick={() => setModalOpen(false)} marginBottom0>
        <FormattedMessage id="ui-ill-ra.button.cancel" />
      </Button>
    </ModalFooter>
  );
  ...
  return <>
    {selected && modalOpen && (
      <Modal
        aria-label={intl.formatMessage({ id: "ui-ill-ra.connector.search.result.placeRequest" })}
        open={modalOpen}
        label={intl.formatMessage({ id: "ui-ill-ra.connector.search.result.placeRequest" })}
        footer={footer}
        size="large"
      >
